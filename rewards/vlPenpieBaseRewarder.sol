...
import "../interfaces/IBribeRewardDistributor.sol";
...
contract vlPenpieBaseRewarder is
    ...
    ReentrancyGuardUpgradeable
{
    ...
    event BribeDistributorUpdated(address _oldDistributor, address _newDistributor);
    ...
    modifier onlymasterPenpie() {
        if (msg.sender != masterPenpie) revert OnlymasterPenpie();
        _;
    }
    ...
    function balanceOf(
        ...
        (uint256 staked, ) = IMasterPenpie(masterPenpie).stakingInfo(
            stakingToken,
            _account
        );
        return staked;
    ...
    function rewardTokenInfosWithBribe(
    ...
    {
        IBribeRewardDistributor.Claimable[] memory claimable = bribeRewardDistributor.getClaimable(_proof);
        uint256 rewardTokensLength = rewardTokens.length + claimable.length;
        bonusTokenAddresses = new address[](rewardTokensLength);
        bonusTokenSymbols = new string[](rewardTokensLength);
    ...
    function allEarnedWithBribe(
        address _account,
        IBribeRewardDistributor.Claim[] calldata _proof
    ) external view returns (uint256[] memory pendingBonusRewards) {
        IBribeRewardDistributor.Claimable[] memory claimable = bribeRewardDistributor.getClaimable(_proof);
        uint256 rewardLength = rewardTokens.length + claimable.length;
        pendingBonusRewards = new uint256[](rewardLength);
        for (uint256 i; i < rewardTokens.length; i++) {
            pendingBonusRewards[i] = earned(_account, rewardTokens[i]);
        }

        for (uint256 i = rewardTokens.length; i < rewardLength; i++) {
            pendingBonusRewards[i] = claimable[i - rewardTokens.length].amount;
        }
    ...
    function updateBribeDistributor(address _bribeRewardDistributor) external onlyOwner {
        address oldDistributor = address(bribeRewardDistributor);
        bribeRewardDistributor = IBribeRewardDistributor(_bribeRewardDistributor);

        emit BribeDistributorUpdated(oldDistributor, _bribeRewardDistributor);
    }    

    function updateRewardQueuer(
        address _rewardManager,
        bool _allowed
    ) external onlyOwner {
    ...
    function queuePenpie(
        uint256 _amount,
        address _account,
        address _receiver
    ) external override onlyManager nonReentrant returns (bool) {
        IERC20(vlPenpie.penpie()).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function _sendReward(
        ...
        uint256 forfeitAmount = _calExpireForfeit(
            _account,
            userRewards[_rewardToken][_account]
        );
    ...
    function _calExpireForfeit(
        address _account,
        uint256 _amount
    ) internal view returns (uint256) {
        uint256 rewardablePercentWAD = vlPenpie.getRewardablePercentWAD(
            _account
        );
